...
@interface LYSPPageMenuLine : UIImageView
@property (nonatomic, copy) void(^hideBlock)(void);

@end

@implementation LYSPPageMenuLine
...
@interface LYSPItem : UIButton

@property (nonatomic, assign) CGFloat imageRatio;
@property (nonatomic, assign) LYSPItemImagePosition imagePosition;
@end

@implementation LYSPItem
...
- (void)initialize {
    _imageRatio = 0.5;
    _imagePosition = LYSPItemImagePositionDefault;
}
...
- (CGRect)imageRectForContentRect:(CGRect)contentRect {
    ...
    switch (self.imagePosition) {
        case LYSPItemImagePositionDefault:
        case LYSPItemImagePositionLeft: {
        ...
        case LYSPItemImagePositionTop: {
            ...
            break;
        case LYSPItemImagePositionRight: {
            _imageRatio = _imageRatio == 0.0 ? 0.5 : _imageRatio;
            CGFloat imageW =  contentRect.size.width * _imageRatio;
            CGFloat imageH = contentRect.size.height;
            ...
            break;
        case LYSPItemImagePositionBottom: {
            _imageRatio = _imageRatio == 0.0 ? 2.0/3.0 : _imageRatio;
            CGFloat imageW =  contentRect.size.width;
            CGFloat imageH = contentRect.size.height * _imageRatio;
...
- (void)setImagePosition:(LYSPItemImagePosition)imagePosition {
...
@property (nonatomic, weak) LYSPItem *functionButton;
@property (nonatomic, weak) CALayer *shadowLine;
@property (nonatomic, strong) NSMutableArray *buttons;
@property (nonatomic, strong) LYSPItem *selectedButton;
...
@implementation LYSPPageMenu


#pragma mark - public

+ (instancetype)pageMenuWithFrame:(CGRect)frame trackerStyle:(LYSPPageMenuTrackerStyle)trackerStyle {
    LYSPPageMenu *pageMenu = [[LYSPPageMenu alloc] initWithFrame:frame trackerStyle:trackerStyle];
    return pageMenu;
}
...
- (void)setItems:(NSArray *)items selectedItemIndex:(NSUInteger)selectedItemIndex {
    ...
    if (self.buttons.count) {
        ...
        LYSPItem *selectedButton = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:selectedButton];
        
        if (self.trackerStyle == LYSPPageMenuTrackerStyleTextZoom) {
...
- (void)removeItemAtIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    if (itemIndex > self.items.count) {return;}
    // 被删除的按钮之后的按钮需要修改tag值
    for (LYSPItem *button in self.buttons) {
    ...
    if (itemIndex < self.buttons.count) {
        LYSPItem *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)setTitle:(NSString *)title image:(UIImage *)image imagePosition:(LYSPItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forItemIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        LYSPItem *button = [self.buttons objectAtIndex:itemIndex];
        [button setTitle:title forState:UIControlStateNormal];
        [button setImage:image forState:UIControlStateNormal];
...
- (void)setFunctionButtonTitle:(NSString *)title image:(UIImage *)image imagePosition:(LYSPItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forState:(UIControlState)state {
...
- (void)addButton:(NSInteger)index object:(id)object animated:(BOOL)animated {
    ...
    LYSPItem *button = [LYSPItem buttonWithType:UIButtonTypeCustom];
...
- (void)initialize {
    ...
    // 必须先添加分割线，再添加backgroundView;假如先添加backgroundView,那也就意味着backgroundView是LYSPPageMenu的第一个子控件,而scrollView又是backgroundView的第一个子控件,当外界在由导航控制器管理的控制器中将LYSPPageMenu添加为第一个子控件时，控制器会不断的往下遍历第一个子控件的第一个子控件，直到找到为scrollView为止,一旦发现某子控件的第一个子控件为scrollView,会将scrollView的内容往下偏移64;这时控制器中必须设置self.automaticallyAdjustsScrollViewInsets = NO;为了避免这样做，这里将分割线作为第一个子控件
    LYSPPageMenuLine *dividingLine = [[LYSPPageMenuLine alloc] init];
    ...
    LYSPItem *functionButton = [LYSPItem buttonWithType:UIButtonTypeCustom];
...
// 按钮点击方法
- (void)buttonInPageMenuClicked:(LYSPItem *)sender {
...
- (void)moveItemScrollViewWithSelectedButton:(LYSPItem *)selectedButton {
...
// 功能按钮的点击方法
- (void)functionButtonClicked:(LYSPItem *)sender {
...
- (void)moveTrackerWithProgress:(CGFloat)progress fromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex currentOffsetX:(CGFloat)currentOffsetX beginOffsetX:(CGFloat)beginOffsetX {
    ...
    if (self.trackerStyle == LYSPPageMenuTrackerStyleLine) {
    ...
    } else if (self.trackerStyle == LYSPPageMenuTrackerStyleLineAttachment){
    ...
    } else if (self.trackerStyle == LYSPPageMenuTrackerStyleTextZoom) {
        // 缩放文字
        [self zoomForTitleWithProgress:progress fromButton:fromButton toButton:toButton];
    } else if (self.trackerStyle == LYSPPageMenuTrackerStyleRoundedRect) {
        newCenter.x = fromButton.center.x + xDistance * progress;
...
- (void)setTrackerStyle:(LYSPPageMenuTrackerStyle)trackerStyle {
    _trackerStyle = trackerStyle;
    switch (trackerStyle) {
        case LYSPPageMenuTrackerStyleLine:
        ...
        case LYSPPageMenuTrackerStyleLineAttachment:
            ...
            break;
        case LYSPPageMenuTrackerStyleRoundedRect:
...
- (void)setSelectedItemIndex:(NSUInteger)selectedItemIndex {
    _selectedItemIndex = selectedItemIndex;
    if (self.buttons.count) {
        LYSPItem *button = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:button];
    }
}

- (void)setDelegate:(id<LYSPPageMenuDelegate>)delegate {
    if (delegate == _delegate) {return;}
    _delegate = delegate;
    if (self.buttons.count) {
        LYSPItem *button = [self.buttons objectAtIndex:_selectedItemIndex];
...
- (void)setPermutationWay:(LYSPPageMenuPermutationWay)permutationWay {
...
- (void)layoutSubviews {
    ...
    for (int i= 0 ; i < self.buttons.count; i++) {
        ...
        if (button.currentTitle && !button.currentImage) {
            ...
            contentW = imageW;
        } else if (button.currentTitle && button.currentImage && (button.imagePosition == LYSPItemImagePositionRight || button.imagePosition == LYSPItemImagePositionLeft)) {
            contentW = textW + imageW;
        } else if (button.currentTitle && button.currentImage && (button.imagePosition == LYSPItemImagePositionTop || button.imagePosition == LYSPItemImagePositionBottom)) {
